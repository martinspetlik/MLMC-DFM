...
class GenTrainSamples:
    """
    Script to generate training samples
    ...
    def __init__(self):
        ...
        self.n_levels = 1  # Number of MLMC levels
    ...
    def run(self, renew=False, recollect=False):
        ...
        if recollect:
            ...
            self.generate_jobs(sampler, n_samples=[10], renew=renew, target_var=1e-4)
    ...
    def setup_config(self, clean):
        ...
        with open(os.path.join(self.work_dir, "sim_config_3D_homogenization_samples.yaml"), "r") as f:
    ...
    def generate_jobs(self, sampler, n_samples=None, renew=False, target_var=None):
        ...
        """
        if len(n_samples) == 1:
            GenTrainSamples.create_zarr_file(n_samples, sampler)
            ...
            if n_samples is not None:
                if self.use_pbs or self.generate_samples_per_level:
                    for level_id in reversed(range(self.n_levels)):
                        ...
                        #                 GenTrainSamples.save_cond_tns_per_coord(level_instance_obj)
                        #
                        #         sample_cond_tns = []
                        #
                        #         directory = level_instance_obj.config_dict["coarse"]["common_files_dir"]
