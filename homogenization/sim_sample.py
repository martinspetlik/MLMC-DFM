...
class DFMSim(Simulation):
    ...
    def calculate(config, seed):
        ...
        #@TODO: save homogenized conductivity for fine samples on coarser level
        # np.save() # append existing data
        ...
        fine_mesh_data = DFMSim.extract_mesh(os.path.join(fine_common_files_dir, DFMSim.MESH_FILE))
        ...
        if coarse_step != 0:
            ...
            coarse_mesh_data = DFMSim.extract_mesh(os.path.join(coarse_common_files_dir, DFMSim.MESH_FILE))
        ...
        fields = DFMSim.make_fields(fields, fine_mesh_data, coarse_mesh_data)
        ...
        fine_input_sample, coarse_input_sample = DFMSim.generate_random_sample(fields, coarse_step=coarse_step,
                                                                               n_fine_elements=len(
        ...
        # Run fine sample
        fields_file = os.path.join(os.getcwd(), DFMSim.FIELDS_FILE)
        fine_res, fine_flow_run_time = DFMSim._run_sample(fields_file, fine_mesh_data['ele_ids'], fine_input_sample,
                                                          flow123d,
                                                          fine_common_files_dir,
                                                          os.path.join(fine_common_files_dir, DFMSim.MESH_FILE))
        ...
        if coarse_input_sample:
            coarse_res, coarse_flow_run_time = DFMSim._run_sample(fields_file, coarse_mesh_data['ele_ids'],
                                                                  coarse_input_sample, flow123d,
                                                                  coarse_common_files_dir,
                                                                  os.path.join(coarse_common_files_dir,
                                                                               DFMSim.MESH_FILE))
    ...
    @staticmethod
    def _run_sample(fields_file, ele_ids, fine_input_sample, flow123d, common_files_dir, mesh_file):
        """
