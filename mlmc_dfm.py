...
class ProcessSimple:
    ...
    def __init__(self):
        ...
        #step_range = [20.0, 1.5]
        step_range = [10.0, 8.325]
        #step_range = [10.0, 1.871]
        #step_range = [10, 0.809]
    ...
    def run(self, renew=False, recollect=False):
        ...
        if recollect:
            ...
            self.generate_jobs(sampler, n_samples=[2, 1,1,1,1], renew=renew)
    ...
    def process(self):
        ...
        location_mean = time_mean['0']  # locations: ['0']
        k_xx = location_mean[1]
        q_value = k_xx #cond_tn_quantity#k_xx
        #q_value = np.array([location_mean[0], location_mean[1], location_mean[2]])

        # @TODO: How to estimate true_domain?
        quantile = 0.001
        true_domain = mlmc.estimator.estimate_domain(q_value, sample_storage, quantile=quantile)
        ...
        if self.n_levels == 1:
            ...
            l_2_samples = estimate_obj.get_level_samples(level_id=2)
            l_2_samples = np.squeeze(l_2_samples)
